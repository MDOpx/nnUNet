import torch
from nnunet.network_architecture.cls_head import ClassificationHead 
from nnunet.network_architecture.up_encoder import CNN_3D_encoder
from nnunet.network_architecture.seg_decoder import Static_UNet
...
class Up_MTL(torch.nn.Module):
    ...
    def __init__(
        ...
        self.num_cls_classes = 1
        self.do_ds = True
        self.final_nonlin = lambda x: x
        self.deep_supervision = True
        
        self.encoder = CNN_3D_encoder(num_classes, in_channels)
        # CLS
        self.classification_head = ClassificationHead(
            in_channels=self.encoder.final_output, 
            out_channels=1,
            pooling='avg', 
            dropout=0.5
        )
        ...
        # SEG
        self.seg_decoder = Static_UNet(self.encoder)

        # REC
        #self.rec_decoder = Res_UNet_rec_decoder(self.encoder)


    def forward(self, x):
        # feature extract
        last_feature, skips = self.encoder(x) #last_feature = 1, 320, 5, 5, 6
        ...
        #Rec
        #rec_decoder_output = self.rec_decoder(last_feature, self.encoder)
       
        #return cls_output, seg_decoder_output, rec_decoder_output
