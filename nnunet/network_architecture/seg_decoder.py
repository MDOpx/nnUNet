...
class Static_UNet(nn.Module):
    ...
    def forward(self, x, skips):
        x_enc5 = x #1, 320, 5, 5, 6
        seg_outputs = []

        x_dec0 = self.upsamplers[0](x_enc5) #x_dec0 = [1, 320, 5, 10, 12]
        x_dec0 = torch.cat((x_dec0, skips[4]), dim=1) # x_dec0 = [1, 640, 5, 10, 12], x_enc4.shape = [1, 320, 5, 10, 12]
        x_dec0 = self.decoders[0](x_dec0) #self.decoders[0] = Conv3dBlock(640, 320, padding=1) -> x_dec0 = [1, 320, 5, 10, 12]
        seg_outputs.append(self.final_nonlin(self.deepsupervision[0](x_dec0))) #self.deepsupervision[0] -> nn.Conv3d(320, 2, kernel_size=1, stride=1, bias=False)
